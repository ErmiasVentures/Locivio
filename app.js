...
function updateDashboard(locations) {
  ...
  if (items) items.innerText = countTotalItems(locations);
  if (favorites) favorites.innerText = countTotalFavorites(locations);
...
function searchItems(talkBack = false) {
  const searchInput = document.getElementById("searchInput");
  ...
  getAllLocations(function (locations) {
    const matches = locations.filter(location =>
      ...
      location.items.some(item => item.toLowerCase().includes(query))
    );
    ...
    if (matches.length === 0) {
      ...
      if (talkBack && typeof speak === "function") {
        speak(message);
      }
